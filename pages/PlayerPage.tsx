...
const PlayerPage: React.FC = () => {
    ...
    const { item: initialItem, type, season: initialSeason, episode: initialEpisode, currentTime, streamUrl, liveChannels, currentChannelIndex, logo } = location.state || {};
    ...
    useEffect(() => {
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                // For live TV, the item might not be in TMDB, so don't show an error
                if (!streamUrl) {
     ...
     useEffect(() => {
        ...
        return () => {
            if (video && item && video.duration > 0 && video.currentTime > 0 && !streamUrl) { // Don't save history for live TV
    ...
    }, [videoNode, item, type, currentSeason, currentEpisode, updateHistory, streamUrl]);
