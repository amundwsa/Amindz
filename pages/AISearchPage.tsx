...
// FIX: Update system instruction to describe the JSON schema textually,
...
You must return ONLY a JSON object with a single key "recommendations", which is an array of up to 3 objects. Each object in the array should have two keys: "title" (a string) and "media_type" (either "movie" or "tv").
Do not add any conversational text or markdown formatting around the JSON object.
...
const AISearchPage: React.FC = () => {
    ...
    const doSearch = async (query: string) => {
        ...
        try {
            ...
            // FIX: Moved `tools` into `config` and removed `responseMimeType` and `responseSchema`
            // to comply with the googleSearch tool usage guidelines.
            ...
            // FIX: Add robust JSON parsing as the response is not guaranteed to be JSON when using googleSearch.
            let jsonResponse;
            try {
